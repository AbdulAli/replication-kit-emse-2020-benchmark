...
//                }
        		...
        		SoftwareVersion testVersion = null;
        		String version = null;
        		int flag = 0;
                for (SoftwareVersion ver : versions) {
                	
                	if(ver.getVersion().contains("typetr")) {
                		version = ver.getVersion();
                		traindataSet.add(ver.getInstances());
                		flag++;
                	} 
                }
                
                for (SoftwareVersion ver : versions) {
                	if(ver.getVersion().contains("typetest") && ver.getVersion().contains(version.split("type")[0])) {
                		testdata = ver.getInstances();
                		testVersion = ver;
                		efforts = ver.getEfforts();
                		flag++;
                	}
                }
                ...
                for (ISetWiseProcessingStrategy processor : config.getSetWisePreprocessors()) {
                    Console.traceln(Level.FINE,
                                    String.format(
                                                  ...
                                                  config.getExperimentName(), versionCount,
                                                  testVersionCount, testVersion.getProject(),
                ...
                for (IEvaluationStrategy evaluator : config.getEvaluators()) {
                    ...
                    evaluator.apply(config.getExperimentName(),testdata, traindata, allTrainers, efforts, writeHeader,
